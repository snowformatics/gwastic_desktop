...
class GWASAI:
    def run_random_forest(self, snp_data, pheno_data, snp_ids):

        snp_data[np.isnan(snp_data)] = -1

        # Split data into training and testing sets
        X_train, X_test, y_train, y_test = train_test_split(snp_data, pheno_data, test_size=0.2, random_state=42)
        ...
        df = pd.DataFrame(data, columns=['SNP', 'feature_importances'])
        return df
